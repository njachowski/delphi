...
###use try loop to catch any bad plots before it crashes program 
plot_result <- try({
  print('trying to plot figure now...')
  p <- ggplot(data=hat, aes(x,y)) + geom_line(color="blue", size=1.5, alpha=.5) + ylim(0,max_normal) + ylab("Normalised Vid    eo Starts (x 100)") + xlab('Months') + ggtitle(this_detail$title_detailed)
  p <- p + geom_point(data=mod, aes(x,y), color="red")
  print(p)
  ggsave(filename = paste0(this_detail$number,'.jpg'), plot = last_plot())
})
if(class(plot_result) == "try-error") {
  print('!!!!PLOTTING FAILURE!!!!!')
} else { print(paste0(this_detail$title_detailed,' SUCCESSFULLY PLOTTED')) }
...
for (i in start_adding:36){
	pred$mo[i] <- i
  try_predict <- try({pred$normal[i] <- yhat[i-1]/100})
  if(class(try_predict) == "try-error"){
    print('!!!FAILED TO MAKE PREDICTION, DEFAULTING TO ZERO')
    pred$normal[i] <- 0
